...
TEST(WhisperOperatorUnitTests, FirstConv1Gelu) {
  ...
  const auto conv1_out = conv1d(input, conv1_weight, conv1_bias);
  const auto result = gelu(conv1_out);

  /* std::cout << "conv1_bias = " << conv1_bias; */
  /* std::cout << "conv1_weight = " << conv1_weight; */
  const auto gold_conv1 = read_binary<float>(gold_1stconv1gelu_fn, 384 * 300);
